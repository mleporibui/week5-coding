...
Mass comparison: male & female chinstrap penquins
...
Get vectors of body mass for male and female chinstraps penguins seperately.
...
```
Are male chinstrap penguins significantly larger (body mas) than female chinstrap penguins?
...
starting from penguins_f creat a summary table that groups by species and returns the following:
mean
sd
sample size
variance 'var'
...
leveneTest(flipper_length_mm~species, data = penguin_f) #levene test for equal variances - car pacakge, to check if variances are close enough
```
...
TukeyHSD(penguin_aov) #posthoc test
```
